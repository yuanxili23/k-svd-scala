...
def computeDistriErr(Y:RowMatrix ,D:BDM[Double], X: RowMatrix):RDD[(Long,BDM[Double])]={
    ...
    var E_indiv=Yindex.join(Xindex).map{case (index, (y,x))=>
        var sum=BDV.zeros[Double](x.size)
        var E_s=BDM.zeros[Double](x.size,D.cols)
        for(i<- 0 until D.cols){
            sum=sum+D(index.toInt,i) *toBreezeV(x)
        }
        for(i<- 0 until D.cols){
          E_s(::,i):=toBreezeV(y)-(sum-D(index.toInt,i)*toBreezeV(x))
        }
        (index,E_s)
    }
    var M=E_indiv.map{case (index, e)=>
        (index,e.map(x=>x*x))
     }
    M
  }
...
def distributed(Y:RowMatrix ,D:BDM[Double], X: RowMatrix, iter:Int):BDM[Double]={

   var Xrows=X.numRows.toInt
   var Xcols=X.numCols.toInt
   var E2=computeDistriErr(Y,D,X)

   var updatedD=BDM.zeros[Double](D.rows, D.cols)

   //create weight matrix
   var W=BDM.rand[Double](D.rows,D.cols)
    //initial Q
   var Q=normalizedCol(BDM.rand(D.rows,D.cols))
   var Zt=E2.map{case(index, m)=> 
    var tmp=BDM.zeros[Double](1, Q.cols)
    for(i<- 0 until Q.cols){
      tmp(0,i)=Q(index.toInt, i)*m(0,i)
    }
    (index, tmp)
   }

   var Qindex:RDD[(Long,BDM[Double])]=null

   for(m<- 0 until iter){
     var Z=Zt.map{case (index,z)=> (index, W*z.t) }
...
//method2
      D=normalizedCol(distributed(A,D,Xrowmatrix,5))
